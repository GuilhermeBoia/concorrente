...
public class ImageMeanFilter {
     ...
     */
    public static void applyMeanFilter(String inputPath, String outputPath, int kernelSize, int num_threads) throws IOException {
        // Load image
        ...
        //Process each part for each thread
        for (int p = 1; p <= num_threads; p++) {
            int start = step * (p-1);
            int end = step * p;
            Thread t = new Thread(() -> processEachPixel(originalImage, filteredImage, outputPath, start, end, width, kernelSize));
            t.start();
        }

    }
    
    private static void processEachPixel(BufferedImage originalImage, BufferedImage filteredImage, String outputPath, int start, int end, int width, int kernelSize) {
    ...
    public static void main(String[] args) {
        ...
        int num_threads = Integer.valueOf(args[1]);

        try {
            applyMeanFilter(inputFile, "filtered_output.jpg", 7, num_threads);
        } catch (IOException e) {
            System.err.println("Error processing image: " + e.getMessage());
        }
    }
